            ...
            <div class="card my-4">
                <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                    <div class="bg-gradient-primary shadow-primary border-radius-lg pt-4 pb-3">
                        <h6 class="text-white text-capitalize ps-3">Cập nhật film</h6>
                    </div>
                    <div class="pst-ab">
                        ...
                        <form action="./index.php?btn_update" enctype="multipart/form-data" method="post" style="display:flex;width:95%;margin:auto">
                            ...
                            <div class="row pb-4">
                                <div class="col-6">
                                    ...
                                    >
                                    <?php
                                    if (!empty($actor_err)) echo $actor_err;
                                    ?>
                                </div>
                                <div class="col-6">
                                    <label for="">Thể loại</label>
                                    <input name="genre" type="text" id="genre" value="<?=$film['genre']?>">
                                    <?php
                                    if (!empty($genre_err)) echo $genre_err;
                                    ?>
                                </div>
                            </div>
                            <div class="row pb-4">
                                <div class="col-4">
                                    ...
                                    <input class="date" name="premiere" type="date" id="" value="<?=$film['premiere']?>">
                                    <?php
                                    if (!empty($premiere_err)) echo $premiere_err;
                                    ?>
                                </div>
                                <div class="col-4">
                                    <label for="">Ngôn ngữ</label>
                                    <input name="language" type="text" id="language" value="<?=$film['language']?>">
                                    <?php
                                    if (!empty($language_err)) echo $language_err;
                                    ?>
                                </div>
                                <div class="col-4">
                                    <label for="">Thời lượng film</label>
                                    <input name="time" type="number" id="time" value="<?=$film['time']?>">
                                    <?php
                            ...
                            <div class="row pb-4">
                                <div class="col-6">
                                    <label for="">Loại tuổi xem</label>
                                    <input name="rated" type="text" id="rated" value="<?=$film['rated']?>">
                                    <?php
                                    if (!empty($rated_err)) echo $rated_err;
                                    ?>
                                </div>
                                <div class="col-6">
                                    <label for="">Link trailer</label>
                                    <input name="trailer" type="text" id="trailer" value="<?=$film['trailer']?>">
                                    <?php
                                    if (!empty($trailer_err)) echo $trailer_err;
                                    ?>
                                </div>
                            </div>
                            <label for="">Mô tả</label>
                            <textarea name="description" id="" cols="30" rows="10" value=""><?=$film['description']?></textarea>
                            ...
                            ?>
                            <div style=" display:block" class="img-product">
                                <img style=" display:block" src="../../uploaded/films/<?=$film['thumb']?>"  id="img-preview">
                            </div>
                            <input name="id_film" type="hidden" value="<?=$film['id']?>">
                            <input name="thumb_old" type="hidden" value="<?=$film['thumb']?>">
    ...
    $(function() {
        $("form").validate({
            ...
            messages: {
                ...
                name: {
                    required: '<div class="text-danger">Chưa điền tên film</div>',
                },
                directors: {
                    required: '<div class="text-danger">Chưa điền đạo diễn</div>',
                },
                actor: {
                    required:'<div class="text-danger">Chưa điền diễn viên</div>',
                },
                genre: {
                    required: '<div class="text-danger">Chưa điền thể loại</div>',
                },
                premiere: {
                    required: '<div class="text-danger">Chưa điền ngày</div>',
                    date: '<div class="text-danger">không đúng định dạng ngày</div>',
                },
                time: {
                    required: '<div class="text-danger">Chưa điền thời gian</div>',
                },
                language: {
                    required: '<div class="text-danger">Chưa điền thời gian</div>',
                },
                rated: {
                    required: '<div class="text-danger">Chưa điền lứa tuổi</div>',
                },
                trailer: {
                    required: '<div class="text-danger">Chưa điền trailer</div>',
                },
                description: {
                    required: '<div class="text-danger">Chưa điền mô tả</div>',
                },
                upload: {
